...
<head>
    ...
    <script async src="https://www.googletagmanager.com/gtag/js?id=G-XXWLVCJHTR"></script>
    <script>
    ...
    <div class="container-fluid">
        ...
        <div class="row row-cols-1">
            <div class="col text-center">
                <img src="../res/hc-180.png" alt="logo-180" width="64" height="64">
            </div>
            <div class="col text-center">
                <div class="fw-bold fs-1" style="color: #ffff00;">GAUHATI HIGH COURT KOHIMA BENCH</div>
            </div>
        </div>
        <div class="row">
            <div class="col text-center">
        ...
        ?>
        <div class="row row row-cols-1 row-cols-md-2 g-2">
            <?php
            if ($display) {
                if (!empty($all_status)) {
                    foreach ($all_status as $c) {
                        if ($c['s_id'] == 3) {
                            ...
                            echo '<div class="text-center fs-1 fw-bold">Court ' . $c['c_number'] . ' (' . $c['ct_name'] . ')</div>';
                            echo '<hr><div class="row fw-bold">
                        <div class="col fs-1 text-center text-light">' . $c['case_name'] . ' - ' . $c['case_regno'] . ' - ' . $c['case_year'] . '</div>
                        </div>';
                            echo '<hr><div class="row fw-bold">
                        <div class="col fs-1 text-center">Item No. </div>
                        <div class="col fs-1 text-center text-light">' . $c['case_sn'] . '</div>
                        </div><hr>';
                            echo '<div class="row fw-bold">
                        <div class="col fs-1 text-center">Status</div>
                        <div class="col fs-1 text-center text-light">' . $c['s_status'] . '</div>
                        </div>';
                            echo '</div></div>';
                            echo '</div>';
                        }
