    ...
    <div class="bg-white p-4 rounded shadow-sm">
        <form action="{{ route('admin.advocate-general.store') }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" name="full_name" placeholder="Enter full name" value="{{ old('full_name') }}" rrr>
                <span class="text-danger small">@error('full_name') {{ $message }} @enderror</span>
            </div>
            
            <div class="mb-3">
                <label class="form-label">Category *</label>
                <x-select :options="$agCategories" name="ag_category_id" class="form-select" placeholder="Select category" :selected="old('ag_category_id')" rrr/>
                <span class="text-danger small">@error('ag_category_id') {{ $message }} @enderror</span>
            </div>
            
            <div class="mb-3">
                <label class="form-label">Date of Joining *</label>
                <input type="text" class="form-control datepicker" name="doj" value="{{ old('doj') }}" rrr>
                <span class="text-danger small">@error('doj') {{ $message }} @enderror</span>
            </div>
            
            <div class="mb-3">
                <label class="form-label">Served Till *</label>
                <input type="text" class="form-control datepicker" name="served_till" value="{{ old('served_till') }}" rrr>
